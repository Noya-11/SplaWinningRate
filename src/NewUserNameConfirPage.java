    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (int i = 0; i < users.length; i++) {
                if (newUserName != users[i]) { // 既に使用されている場合。

                } else if(newUserName == users[i]){
                    
                    
                    out.println("<b>既に使用されているユーザー名です。</b>");
                    out.println("<a href='/SplatoonRecode/RegistrationPage.html'>登録ぺージへ戻る</a>");
                    registFlag = false;
                    //テーブル作成入力されたユーザー名使って、ここから始める。
                }
            }
